...
QImageViewer::QImageViewer(QWidget *parent)/* : QWidget(parent)*/
{
    ...
    fileChose = new qFileChose(this->parent);
    initImageResource();
}
...
int QImageViewer::loadImageResource(void)
{
    ...
    //        "D:\\Documents\\Pictures", tr("Images (*.jpg *.jpeg *.png *.bmp *.gif)"),0, QFileDialog::DontUseNativeDialog|QFileDialog::ReadOnly);
    /* 这里重写了系统的默认打开文件对话框，因为系统打开文件对话框太大！，4.3寸 480*272屏不能完全显示，所以就重写了*/
    filename = fileChose->choseOpenFileName(this->parent,"选择文本文件", "./Resource/", tr("Images (*.jpg *.jpeg *.png *.bmp *.gif)"),QFileDialog::DontUseNativeDialog|QFileDialog::ReadOnly);
    if (filename.isEmpty()) {
        return -1;
    }
...
int QImageViewer::loadImageResource(const QString &caption,
                                    ...
                                    const QString &filer)
{
    filename = fileChose->choseOpenFileName(this->parent,caption, directory, filer,QFileDialog::DontUseNativeDialog|QFileDialog::ReadOnly);
    //filename = QFileDialog::getOpenFileName(this,caption,directory,filer,0, QFileDialog::DontUseNativeDialog|QFileDialog::ReadOnly);
    if (filename.isEmpty()) {
        return -1;
    }
